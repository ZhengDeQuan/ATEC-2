...
from propertyAndTransPreprocess import load_property
import random
import jieba.posseg as pseg


class ARNN:
    def __init__(self,embedding, property):
    ...
    def build_cross_encoding_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        with tf.variable_scope('placeholders'):
            self.input_q = tf.placeholder(tf.int32, [None, None], name='input_q')  # placeholder只存储一个batch的数据
            self.input_q_pro = tf.placeholder(tf.int32, [None, None], name='input_q_pro')
            self.input_r = tf.placeholder(tf.int32, [None, None], name='input_r')  # placeholder只存储一个batch的数据
            self.input_r_pro = tf.placeholder(tf.int32, [None, None], name='input_r_pro')
            ...
            self.property_ph = tf.placeholder(tf.float32, shape=(42, 41))
        ...
        with tf.variable_scope('word_embedding'):
            ...
            word_property = tf.get_variable('word_property', shape=(42, 41), dtype=tf.float32)
            self.embedding_init = word_embeddings.assign(self.embedding_ph)
            self.property_init = word_property.assign(self.property_ph)
            ...
            q_property = tf.nn.embedding_lookup(word_property, self.input_q_pro)
            r_property = tf.nn.embedding_lookup(word_property, self.input_r_pro)
            q_embedding = tf.concat((q_embedding, q_property), axis=2)
            r_embedding = tf.concat((r_embedding, r_property), axis=2)
        ...
        if training:
            i = 0
            while os.path.exists('./crosspro_trans' + str(i)):
                i += 1
            os.makedirs('./crosspro_trans' + str(i))
            return './crosspro_trans' + str(i)


    def build_direct_encoding_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        with tf.variable_scope('placeholders'):
            self.input_q = tf.placeholder(tf.int32, [None, None], name='input_q')  # placeholder只存储一个batch的数据
            self.input_q_pro = tf.placeholder(tf.int32, [None, None], name='input_q_pro')
            self.input_r = tf.placeholder(tf.int32, [None, None], name='input_r')  # placeholder只存储一个batch的数据
        ...
        if training:
            i = 0
            while os.path.exists('./directpro_trans' + str(i)):
                i += 1
            os.makedirs('./directpro_trans' + str(i))
            return './directpro_trans' + str(i)


    def build_slice_encoding_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        with tf.variable_scope('placeholders'):
            self.input_q = tf.placeholder(tf.int32, [None, None], name='input_q')  # placeholder只存储一个batch的数据
            self.input_q_pro = tf.placeholder(tf.int32, [None, None], name='input_q_pro')
            self.input_r = tf.placeholder(tf.int32, [None, None], name='input_r')  # placeholder只存储一个batch的数据
        ...
        if training:
            i = 0
            while os.path.exists('./slicepro_trans' + str(i)):
                i += 1
            os.makedirs('./slicepro_trans' + str(i))
            return './slicepro_trans' + str(i)


    def build_base_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        with tf.variable_scope('placeholders'):
            self.input_q = tf.placeholder(tf.int32, [None, None], name='input_q')  # placeholder只存储一个batch的数据
            self.input_q_pro = tf.placeholder(tf.int32, [None, None], name='input_q_pro')
            self.input_r = tf.placeholder(tf.int32, [None, None], name='input_r')  # placeholder只存储一个batch的数据
        ...
        if training:
            i = 0
            while os.path.exists('./basepro_trans' + str(i)):
                i += 1
            os.makedirs('./basepro_trans' + str(i))
            return './basepro_trans' + str(i)


    def build_interaction_cnn_model(self,rnn_units=200 + 41,interaction_dims=8,training=True):
        with tf.variable_scope('placeholders'):
            self.input_q = tf.placeholder(tf.int32, [None, None], name='input_q')  # placeholder只存储一个batch的数据
            self.input_q_pro = tf.placeholder(tf.int32, [None, None], name='input_q_pro')
            self.input_r = tf.placeholder(tf.int32, [None, None], name='input_r')  # placeholder只存储一个batch的数据
        ...
        if training:
            i = 0
            while os.path.exists('./icnnpro_trans' + str(i)):
                i += 1
            os.makedirs('./icnnpro_trans' + str(i))
            return './icnnpro_trans' + str(i)


    def build_gru_cnn_model(self, rnn_units=200 + 41, interaction_dims=8, training=True):
        with tf.variable_scope('placeholders'):
            self.input_q = tf.placeholder(tf.int32, [None, self.max_cut_len], name='input_q')  # placeholder只存储一个batch的数据
            self.input_q_pro = tf.placeholder(tf.int32, [None, self.max_cut_len], name='input_q_pro')
            self.input_r = tf.placeholder(tf.int32, [None, self.max_cut_len], name='input_r')  # placeholder只存储一个batch的数据
            self.input_r_pro = tf.placeholder(tf.int32, [None, self.max_cut_len], name='input_r_pro')
            self.q_sequence_len = tf.placeholder(tf.int32, [None], name='q_sequence_len')
            self.r_sequence_len = tf.placeholder(tf.int32, [None], name='r_sequence_len')
            self.input_y = tf.placeholder(tf.int32, [None], name='input_y')
            self.embedding_ph = tf.placeholder(tf.float32, shape=(self.total_words, self.word_embedding_size))
        ...
        if training:
            i = 0
            while os.path.exists('./grucnnpro_trans' + str(i)):
                i += 1
            os.makedirs('./grucnnpro_trans' + str(i))
            return './grucnnpro_trans' + str(i)
    ...
    def predict(self,model_path,data_q, data_q_pro, data_r, data_r_pro):
        ...
        with tf.Session(config=config) as sess:
            ...
            while True:
                ...
                batch_q_pro = pad_sequences(data_q_pro[low:low + n_sample], padding='post')
                ...
                batch_r_pro = pad_sequences(data_r_pro[low:low + n_sample], padding='post')
                feed_dict = {
                    self.input_q: np.array(batch_q),
                    self.input_q_pro: np.array(batch_q_pro),
                    self.q_sequence_len: np.array(batch_q_len),
                    self.input_r: np.array(batch_r),
                    self.input_r_pro: np.array(batch_r_pro),
    ...
    def predict_cut(self,model_path,data_q, data_q_pro, data_r, data_r_pro):
        saver = tf.train.Saver()
        config = tf.ConfigProto()
        config.gpu_options.per_process_gpu_memory_fraction = 0.95  # 只分配40%的显存
        all_pred_label=[]
        ...
        with tf.Session(config=config) as sess:
            ...
            while True:
                ...
                batch_q = pad_sequences(data_q[low:low + n_sample], padding='post', maxlen=self.max_cut_len)
                batch_q_pro = pad_sequences(data_q_pro[low:low + n_sample], padding='post', maxlen=self.max_cut_len)
                batch_r_len = self.get_sequences_length(data_r[low:low + n_sample], maxlen=self.max_cut_len)
                batch_r = pad_sequences(data_r[low:low + n_sample], padding='post', maxlen=self.max_cut_len)
                batch_r_pro = pad_sequences(data_r_pro[low:low + n_sample], padding='post', maxlen=self.max_cut_len)
                feed_dict = {
                    self.input_q: np.array(batch_q),
                    self.input_q_pro: np.array(batch_q_pro),
                    self.q_sequence_len: np.array(batch_q_len),
    ...
    def evaluate_val_for_train(self, sess, data):
        val_q, val_pro_q, val_r, val_pro_r, val_labels = data
        all_pred_label = []
        low = 0
        batch_size_for_val=1000
        while True:
            ...
            batch_pro_q = pad_sequences(self.copy_list(val_pro_q[low:low + n_sample]), padding='post')
            ...
            batch_pro_r = pad_sequences(self.copy_list(val_pro_r[low:low + n_sample]), padding='post')
            feed_dict = {
                self.input_q: np.array(batch_q),
                self.input_q_pro: np.array(batch_pro_q),
                self.q_sequence_len: np.array(batch_q_len),
                self.input_r: np.array(batch_r),
                self.input_r_pro: np.array(batch_pro_r),
                self.r_sequence_len: np.array(batch_r_len),
    ...
    def evaluate_val_for_train_cut(self, sess, data):
        ...
        while True:
            ...
            batch_pro_q = pad_sequences(self.copy_list(val_pro_q[low:low + n_sample]), padding='post', maxlen=self.max_cut_len)
            ...
            batch_pro_r = pad_sequences(self.copy_list(val_pro_r[low:low + n_sample]), padding='post', maxlen=self.max_cut_len)
            feed_dict = {
                self.input_q: np.array(batch_q),
                self.input_q_pro: np.array(batch_pro_q),
                self.q_sequence_len: np.array(batch_q_len),
    ...
    def train_model_with_random_sample(self, file_src_dict, store_path, continue_train=False,
        ...
        # prepare data for val:
        with open(file_src_dict['evaluate_file'], 'rb') as f:
            val_q, val_pro_q, val_r, val_pro_r, val_labels = pickle.load(f)
        val_data=[val_q, val_pro_q, val_r, val_pro_r, val_labels]
        with tf.Session(config=config) as sess:
            file=open(store_path+'/record.txt','w+')
            train_writer = tf.summary.FileWriter(store_path, sess.graph)
            # prepare data for train:
            with open(file_src_dict['train_random_file'], 'rb') as f:
                train_q, train_pro_q, train_r, train_pro_r = pickle.load(f)
            with open(file_src_dict['all_utterances'],'rb')  as f:
                all_utterances=pickle.load(f)
            with open(file_src_dict['all_utterances_pro'], 'rb') as f:
                all_utterances_pro = pickle.load(f)
            with open(file_src_dict['train_fix_file'], 'rb') as f:
                fix_q, fix_pro_q, fix_r, fix_pro_r, fix_labels = pickle.load(f)
            all_utterances_len=self.get_sequences_length(all_utterances, maxlen=self.max_sentence_len)
            if continue_train is False:
                sess.run(init)
                sess.run(self.embedding_init, feed_dict={self.embedding_ph: self.embedding})
                sess.run(self.property_init, feed_dict={self.property_ph: self.property})
            else:
                saver.restore(sess, previous_model_path)
            low = 0
            epoch = 1
            ...
            while epoch < self.epoch_num:
                ...
                tmp_pro = np.array(all_utterances_pro)
                negs = [list(tmp[negative_indices[i]]) for i in range(negative_samples)]
                negs_pro = [list(tmp_pro[negative_indices[i]]) for i in range(negative_samples)]
                ...
                new_batch_pro_q = train_pro_q[low:low + n_sample]
                new_batch_q_len=self.copy_list(batch_q_len)
                new_batch_r=train_r[low:low + n_sample]
                new_batch_pro_r = train_pro_r[low:low + n_sample]
                new_batch_r_len=self.copy_list(batch_r_len)
                for neg, neg_pro, neg_len in zip(negs, negs_pro, negs_len):
                    new_batch_q += neg
                    new_batch_pro_q += neg_pro
                ...
                batch_fix_pro_q = fix_pro_q[low_fix:low_fix + n_sample * n_times]
                ...
                batch_fix_pro_r = fix_pro_r[low_fix:low_fix + n_sample * n_times]
                new_batch_q+=batch_fix_q
                new_batch_r+=batch_fix_r
                new_batch_pro_q += batch_fix_pro_q
                new_batch_pro_r += batch_fix_pro_r
                new_batch_q+=batch_fix_r
                new_batch_r+=batch_fix_q
                new_batch_pro_q += batch_fix_pro_r
                new_batch_pro_r += batch_fix_pro_q
                ...
                new_batch_pro_q = pad_sequences(self.copy_list(new_batch_pro_q), padding='post')
                new_batch_pro_r = pad_sequences(self.copy_list(new_batch_pro_r), padding='post')
                feed_dict = {
                    self.input_q: np.array(new_batch_q),
                    self.input_q_pro: np.array(new_batch_pro_q),
                    self.q_sequence_len: np.array(new_batch_q_len),
                    self.input_r: np.array(new_batch_r),
                    self.input_r_pro: np.array(new_batch_pro_r),
    ...
    def train_model_with_random_sample_random(self, file_src_dict, store_path, continue_train=False,
        ...
        with tf.Session(config=config) as sess:
            ...
            with open(file_src_dict['train_fix_file'], 'rb') as f:
                fix_q, fix_pro_q, fix_r, fix_pro_r, fix_labels = pickle.load(f)
            with open(file_src_dict['train_trans_file'], 'rb') as f:
                trans_q, trans_pro_q, trans_r, trans_pro_r, trans_labels = pickle.load(f)
            all_utterances_len=self.get_sequences_length(all_utterances, maxlen=self.max_sentence_len)
            if continue_train is False:
                sess.run(init)
                sess.run(self.embedding_init, feed_dict={self.embedding_ph: self.embedding})
            ...
            while epoch < self.epoch_num:
                ...
                fix_pro_q_tmp = np.array(fix_pro_q)
                batch_fix_q_len = self.get_sequences_length(fix_q_tmp[random_fix], maxlen=self.max_sentence_len)
                batch_fix_q = list(fix_q_tmp[random_fix])
                batch_fix_pro_q = list(fix_pro_q_tmp[random_fix])
                fix_r_tmp = np.array(fix_r)
                fix_pro_r_tmp = np.array(fix_pro_r)
                batch_fix_r_len = self.get_sequences_length(fix_r_tmp[random_fix], maxlen=self.max_sentence_len)
                batch_fix_r = list(fix_r_tmp[random_fix])
                batch_fix_pro_r = list(fix_pro_r_tmp[random_fix])
                fix_labels_tmp = np.array(fix_labels)
                batch_fix_labels = list(fix_labels_tmp[random_fix])

                # get random trans data
                random_fix = np.random.randint(0, len(trans_q), n_sample)
                trans_q_tmp = np.array(trans_q)
                trans_pro_q_tmp = np.array(trans_pro_q)
                batch_trans_q_len = self.get_sequences_length(trans_q_tmp[random_fix], maxlen=self.max_sentence_len)
                batch_trans_q = list(trans_q_tmp[random_fix])
                batch_trans_pro_q = list(trans_pro_q_tmp[random_fix])
                trans_r_tmp = np.array(trans_r)
                trans_pro_r_tmp = np.array(trans_pro_r)
                batch_trans_r_len = self.get_sequences_length(trans_r_tmp[random_fix], maxlen=self.max_sentence_len)
                batch_trans_r = list(trans_r_tmp[random_fix])
                batch_trans_pro_r = list(trans_pro_r_tmp[random_fix])
                trans_labels_tmp = np.array(trans_labels)
                batch_trans_labels = list(trans_labels_tmp[random_fix])
                ...
                new_batch_q += batch_trans_q
                new_batch_r += batch_trans_r
                new_batch_pro_q += batch_trans_pro_q
                new_batch_pro_r += batch_trans_pro_r
                new_batch_q += batch_trans_r
                new_batch_r += batch_trans_q
                new_batch_pro_q += batch_trans_pro_r
                new_batch_pro_r += batch_trans_pro_q
                new_batch_q_len += batch_trans_q_len
                new_batch_r_len += batch_trans_r_len
                new_batch_q_len += batch_trans_r_len
                new_batch_r_len += batch_trans_q_len
                batch_label += batch_trans_labels
                batch_label += batch_trans_labels
    ...
    def train_model_with_random_sample_random_cut(self, file_src_dict, store_path, continue_train=False,
        ...
        with tf.Session(config=config) as sess:
            ...
            while epoch < self.epoch_num:
                ...
                fix_q_tmp = np.array(fix_q)
                fix_pro_q_tmp = np.array(fix_pro_q)
                batch_fix_q_len = self.get_sequences_length(fix_q_tmp[random_fix], maxlen=self.max_cut_len)
                batch_fix_q = list(fix_q_tmp[random_fix])
                batch_fix_pro_q = list(fix_pro_q_tmp[random_fix])
                fix_r_tmp = np.array(fix_r)
                fix_pro_r_tmp = np.array(fix_pro_r)
                batch_fix_r_len = self.get_sequences_length(fix_r_tmp[random_fix], maxlen=self.max_cut_len)
                batch_fix_r = list(fix_r_tmp[random_fix])
                batch_fix_pro_r = list(fix_pro_r_tmp[random_fix])
                fix_labels_tmp = np.array(fix_labels)
                batch_fix_labels = list(fix_labels_tmp[random_fix])
                ...
                trans_q_tmp = np.array(trans_q)
                trans_pro_q_tmp = np.array(trans_pro_q)
                batch_trans_q_len = self.get_sequences_length(trans_q_tmp[random_fix], maxlen=self.max_cut_len)
                batch_trans_q = list(trans_q_tmp[random_fix])
                batch_trans_pro_q = list(trans_pro_q_tmp[random_fix])
                trans_r_tmp = np.array(trans_r)
                trans_pro_r_tmp = np.array(trans_pro_r)
                batch_trans_r_len = self.get_sequences_length(trans_r_tmp[random_fix], maxlen=self.max_cut_len)
                batch_trans_r = list(trans_r_tmp[random_fix])
                batch_trans_pro_r = list(trans_pro_r_tmp[random_fix])
                trans_labels_tmp = np.array(trans_labels)
                batch_trans_labels = list(trans_labels_tmp[random_fix])
                ...
                new_batch_pro_q = pad_sequences(self.copy_list(new_batch_pro_q), padding='post', maxlen=self.max_cut_len)
                new_batch_pro_r = pad_sequences(self.copy_list(new_batch_pro_r), padding='post', maxlen=self.max_cut_len)
                feed_dict = {
                    self.input_q: np.array(new_batch_q),
                    self.input_q_pro: np.array(new_batch_pro_q),
                    self.q_sequence_len: np.array(new_batch_q_len),
...
def train_onehotkey():
    print('start')
    file_src_dict = {'embedding_file': './data3/word_embedding.pkl','train_random_file':'./data3/random_train.pkl','evaluate_file':'./data3/val.pkl',
                     'all_utterances':'./data3/all_utterances','train_fix_file':'./data3/train.pkl', 'all_utterances_pro':'./data3/all_utterances_pro',
                     'property':'./data3/property.pkl', 'train_trans_file': './data3/trans_train.pkl'}
    with open(file_src_dict['embedding_file'], 'rb') as f:  # embedding is a 2d-list with size :vocab_size*dim
        embeddings = pickle.load(f)
    with open(file_src_dict['property'], 'rb') as f:
        property = pickle.load(f)
    arnn = ARNN(embedding=embeddings, property=np.array(property))
    print('build graph')
...
def process(inpath,outpath):
    ...
    vocab_hash=pickle.load(open('./data3/word_dict.pkl','rb'))
    with open(inpath, 'r') as fin, open(outpath, 'w') as fout:
        ...
        for line in fin:
            ...
            words = []
            flags = []
            word_and_flag = pseg.cut(sen1)
            for word, flag in word_and_flag:
                if (flag in property_index) and (not word.isspace()):
                    words.append(word)
                    flags.append(property_index[flag])
            for i in range(len(words)):
                if words[i] in vocab_hash:
                    index_q.append(vocab_hash[words[i]])
                    index_q_pro.append(flags[i])
            words = []
            flags = []
            word_and_flag = pseg.cut(sen2)
            for word, flag in word_and_flag:
                if (flag in property_index) and (not word.isspace()):
                    words.append(word)
                    flags.append(property_index[flag])
            for i in range(len(words)):
                if words[i] in vocab_hash:
                    index_r.append(vocab_hash[words[i]])
                    index_r_pro.append(flags[i])
            data_q.append(index_q)
            data_q_pro.append(index_q_pro)
            data_r.append(index_r)
            data_r_pro.append(index_r_pro)
        result=ensemble(data_q, data_q_pro, data_r, data_r_pro)
...
def ensemble(val_q, val_q_pro, val_r, val_r_pro, val_label=None):
    file_src_dict = {'embedding_file': './data3/word_embedding.pkl', 'property': './data3/property.pkl'}
    with open(file_src_dict['embedding_file'], 'rb') as f:  # embedding is a 2d-list with size :vocab_size*dim
        embeddings = pickle.load(f)
    with open(file_src_dict['property'], 'rb') as f:
        propertys = pickle.load(f)
    all_pred_score=[]
    all_pred_label=[]
    ...
    #models = ['./crosspro0/model.3', './crosspro0/model_best.5120', './directpro0/model.4', './directpro0/model.5',
    #          './slicepro0/model.4', './slicepro0/model_best.92160', './basepro0/model.4', './basepro0/model.5',
    #          './icnnpro0/model.6', './icnnpro0/model.7', './grucnnpro0/model.4', './grucnnpro0/model.5']
    #model_name = ['crosspro', 'crosspro', 'directpro', 'directpro', 'slicepro', 'slicepro',
    #              'basepro', 'basepro', 'icnnpro', 'icnnpro', 'grucnnpro', 'grucnnpro']
    models = ['./crosspro_trans2/model.3', './crosspro_trans2/model.5']
    model_name = ['crosspro_trans', 'crosspro_trans']
    graphs = [tf.Graph() for i in range(0, len(models))]
    for i in range(0,len(graphs)):
        with graphs[i].as_default():
            arnn = ARNN(embedding=embeddings, property=propertys)
            if model_name[i]=='crosspro_trans':
                arnn.build_cross_encoding_model(training=False)
            elif model_name[i]=='slicepro_trans':
                arnn.build_slice_encoding_model(training=False)
            elif model_name[i]=='directpro_trans':
                arnn.build_direct_encoding_model(training=False)
            elif model_name[i]=='icnnpro_trans':
                arnn.build_interaction_cnn_model(training=False)
            elif model_name[i]=='basepro_trans':
                arnn.build_base_model(training=False)
            elif model_name[i] == 'grucnnpro_trans':
                arnn.build_gru_cnn_model(training=False)

            if model_name[i] == 'grucnnpro_trans':
                pred_label, pred_score = arnn.predict_cut(model_path=models[i], data_q=val_q, data_q_pro=val_q_pro, data_r=val_r, data_r_pro=val_r_pro)
            else:
                pred_label, pred_score = arnn.predict(model_path=models[i], data_q=val_q, data_q_pro=val_q_pro, data_r=val_r, data_r_pro=val_r_pro)
