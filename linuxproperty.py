...
In uniform distribution this ends up being the range: x = sqrt(6. / (in + out)); [-x, x] 
and for normal distribution a standard deviation of sqrt(2. / (in + out)) is used.
'''
...
from WordpropertyPreprocess import load_property
import random
...
class ARNN:
    def __init__(self,embedding, property):
        ...
        self.max_cut_len = 40
        self.l2=0.0
        self.kp=1.0 #keep_prob


    def build_cross_encoding_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        #用了r_mebedding 和 q_gru的interaction
        #用了q_mebedding 和 r_gru的interaction
        with tf.variable_scope('placeholders'):
            ...
            self.property_ph = tf.placeholder(tf.float32, shape=(42, 41)) # 41种单词的属性，为什么有42行首行全0 作为others的标记
        ...
        with tf.variable_scope('attention_and_interaction'):
            ...
            reqg_attention=tf.matmul(left,right) # r_embedding * attention_weight * q_gru  --> [batch , seq_r ,seq_q]
            reqg_attention=tf.nn.softmax(reqg_attention)#[batch , seq_r , seq_q ] #在seq_q这个维度上归一化
            reqg_q_vector=tf.matmul(reqg_attention,q_gru) #利用r_embedding作为注意力的权重，对q加权平均 #[batch , seq_r , rnn]

            ileft=tf.tensordot(r_embedding,interaction_weight,axes=(2,0)) #[batch , seq_r , rnn] * [rnn , inter_dim , rnn] --> [batch , seq_r , inter_dim , rnn ]# r的[1,2]维与inter的[0,1]维进行矩阵相乘 [batch ,seq_r , rnn] * [rnn,inter_dim , rnn] = [batch , seq_r , inter_dim , rnn]
            ileft=tf.expand_dims(ileft,axis=3) #[batch , seq_r , inter_dim , rnn] --> [batch , seq_r , inter_dim , 1 , rnn]

            iright=tf.stack([reqg_q_vector]*interaction_dims,axis=2) #[1 , batch , seq_r , rnn] * 16 -->[16 , batch , seq_r , rnn] 在rnn这个维度上进行组合-->[batch , seq_r , 16 , rnn]
            iright=tf.expand_dims(iright,axis=4)#[batch , seq_r , 16 , rnn , 1]
            reqg_interaction=tf.matmul(ileft,iright)[:,:,:,-1,-1] #[batch , seq_r , 16 , 1 , 1] --> [batch , seq_r , 16] #按照当前的r_mebedding的选择，对q进行的加权平均，得到的结果在与r计算一个rT*W*q的特征，这种特征要算16个，这个便是interaction的操作，要熟悉tf的API才能写出这个的
        ...
        if training:
            i = 0
            while os.path.exists('./crosspro' + str(i)):
                i += 1
            os.makedirs('./crosspro' + str(i))
            return './crosspro' + str(i)
    ...
    def build_direct_encoding_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        ...
        if training:
            i = 0
            while os.path.exists('./directpro' + str(i)):
                i += 1
            os.makedirs('./directpro' + str(i))
            return './directpro' + str(i)
    ...
    def build_slice_encoding_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        ...
        if training:
            i = 0
            while os.path.exists('./slicepro' + str(i)):
                i += 1
            os.makedirs('./slicepro' + str(i))
            return './slicepro' + str(i)
    ...
    def build_base_model(self,rnn_units=200 + 41,interaction_dims=16,training=True):
        ...
        if training:
            i = 0
            while os.path.exists('./basepro' + str(i)):
                i += 1
            os.makedirs('./basepro' + str(i))
            return './basepro' + str(i)
    ...
    def build_interaction_cnn_model(self,rnn_units=200 + 41,interaction_dims=8,training=True):
        ...
        if training:
            i = 0
            while os.path.exists('./icnnpro' + str(i)):
                i += 1
            os.makedirs('./icnnpro' + str(i))
            return './icnnpro' + str(i)
    ...
    def build_gru_cnn_model(self, rnn_units=200 + 41, interaction_dims=8, training=True):
        ...
        if training:
            i = 0
            while os.path.exists('./grucnnpro' + str(i)):
                i += 1
            os.makedirs('./grucnnpro' + str(i))
            return './grucnnpro' + str(i)
...
def train_onehotkey():
    print('start')
    file_src_dict = {'embedding_file': './data2/word_embedding.pkl','train_random_file':'./data2/random_train.pkl','evaluate_file':'./data2/val.pkl',
                     'all_utterances':'./data2/all_utterances','train_fix_file':'./data2/train.pkl', 'all_utterances_pro':'./data2/all_utterances_pro', 'property':'./data2/property.pkl'}
...
def process(inpath,outpath):
    ...
    vocab_hash=pickle.load(open('./data2/word_dict.pkl','rb'))
...
def ensemble(val_q, val_q_pro, val_r, val_r_pro, val_label=None):
    file_src_dict = {'embedding_file': './data2/word_embedding.pkl', 'property': './data2/property.pkl'}
    with open(file_src_dict['embedding_file'], 'rb') as f:  # embedding is a 2d-list with size :vocab_size*dim
        embeddings = pickle.load(f)
    with open(file_src_dict['property'], 'rb') as f:
        propertys = pickle.load(f) #41种词性的独热编码的数组
    all_pred_score=[]
    all_pred_label=[]
